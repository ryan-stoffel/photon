...
- The first-run sequence is a borderless 800×560 window (`OnboardingWindow`), centered, with 16 pt corners and a soft shadow. The desktop stays visible. Timings live in `OnboardingTiming`. Sora is bundled under `Resources/Fonts` and registered at launch. Revision 4 of `onboardingRevision` is this windowed sequence; Settings > General resets it. While the native parity harness is attached, the sequence opens on a settled frame so the beam does not block CI. Accessibility and Input Monitoring are requested from their own screens. The try-it step listens for the real launcher hotkey, opens the panel, and plays a muted particle burst inside the onboarding window.
