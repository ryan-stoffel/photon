...
- Settings is a System Settings–style SwiftUI `NavigationSplitView` (sidebar + grouped `Form` detail) in both the `Settings` scene and the AppKit fallback window. ⌘, is wired through the Settings scene, the Photon menu, the status item, and a local key monitor so it works while the launcher is key. Panes: General, Appearance, Clipboard, Notes, Files, Keybinds, About. `PHOTON_DEBUG=1` enables cheap `PhotonTiming` logs for launcher show/search and Files search.
