...
- The first-run sequence is a borderless 800×560 window (`OnboardingWindow`), centered, with continuous 16 pt corners and the native window shadow. The desktop stays visible. Every duration and motion constant lives in `OnboardingTiming`. Sora is bundled under `Resources/Fonts` and registered at launch. The backdrop (`OnboardingBackdrop`) is a SwiftUI `Canvas`: black, a slow nebula of soft gradients, and faint drifting stars; it sits under every step and never resets. The reveal (`OnboardingRevealView`) draws the beam, collapse, and flash additively in a 60 fps `Canvas`, then the mark scales in as the flash decays. Steps crossfade and slide 24 pt horizontally on a spring. Permission and feature steps share `OnboardingStepLayout`: one glyph, one headline, one line, one primary button. Accessibility and Input Monitoring are requested from their own screens; Skip for now shows what will not work. The try-it step draws physical key caps (`OnboardingKeycap`) for the configured hotkey, waits for the real launcher hotkey, opens the panel, and plays a SpriteKit confetti burst (`OnboardingConfetti`, a hand-rolled integrator with gravity, drag, spin, and flutter) inside the onboarding window only. Escape skips any step except permissions. Reduce Motion swaps the beam, flash, and confetti for crossfades. Revision 5 of `onboardingRevision` is this sequence; Settings > General resets it. While the native parity harness is attached, every curve renders at its settled end so screenshots are stable.
