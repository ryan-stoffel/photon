...
- The first-run sequence is a borderless full-screen window (`OnboardingWindow`), not a settings sheet. Timings live in `OnboardingTiming`. Sora is bundled under `Resources/Fonts` and registered at launch. Revision 3 of `onboardingRevision` is this sequence; Settings > General resets it. While the native parity harness is attached, the sequence opens on a settled frame so the beam does not block CI. Accessibility and Input Monitoring are requested from their own screens. The try-it step listens for the real launcher hotkey, opens the panel, and plays a short confetti burst around it.
